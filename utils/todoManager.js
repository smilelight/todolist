...
class TodoManager {
  ...
  _timeOrder(a,b) {
    ...
    return parseInt(miniteA) < parseInt(miniteB) && parseInt(secondA) < parseInt(secondB)
  }

  _dateOrder(a,b) {
    let yearA,monthA,dayA = a.date.split("/")
    let yearB,monthB,dayB = b.date.split("/")
    return parseInt(yearA) <parseInt(yearB) && parseInt(monthA) < parseInt(monthB) && parseInt(dayA) < parseInt(dayB)
  }

  _dateAndtimeOrder(a,b) {
    return this._dateOrder(a,b) && this._timeOrder(a,b)
  }
}
